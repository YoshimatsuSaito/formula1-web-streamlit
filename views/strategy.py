...
def _get_optimization_results(
    ...
    dict_tyre_combi: dict[int, list[str]],
) -> pd.DataFrame:
...
def _create_list_pace(strat_data: pd.Series) -> list[float]:
    """Create list of pace at each lap."""
    list_pace = list()
    lap_in_total = 0
    ...
    for stint in range(1, n_stop + 2):
        ...
        for lap in range(laps_stint):
            ...
            pace = (
                pace_stint
                + (degradation_stint * lap)
                + pitloss_lap
                - (lap_in_total * (track_improvement + fuel_effect))
            )
...
def _show_pace_graph(df_res: pd.DataFrame) -> None:
    """Show pace graph."""
    plot_area = st.empty()
    ...
    df_res_sorted.insert(
        0,
        "Strategy",
        df_res_sorted["tyre_1"].str[:1]
        + df_res_sorted["tyre_2"].str[:1]
        + df_res_sorted["tyre_3"].str[:1]
        + df_res_sorted["tyre_4"].str[:1],
    )
    df_res_sorted.index = df_res_sorted["Strategy"]
    df_res_sorted.insert(
        0,
        "diff",
        df_res_sorted["total_lap_time"] - df_res_sorted["total_lap_time"].min(),
    )

    fig, ax = plt.subplots(nrows=3, dpi=250, figsize=(8, 8))
    for n_idx, idx, ls in zip(
        range(2, -1, -1), df_res_sorted.index[:3], ["solid", "dashed", "dotted"]
    ):
        strat_data = df_res_sorted.loc[idx]
        list_pace = _create_list_pace(strat_data)
        for stint in range(1, strat_data["n_stop"] + 2):
            ...
            if stint == strat_data["n_stop"] + 1:
                    ...
                    start + strat_data[f"laps_{stint}"] + 1
                )  # extend line to start of next stint
            ax[1].plot(
                range(start, end),
                list_pace[start:end],
                color=dict_color_tyre[strat_data[f"tyre_{stint}"]],
                linewidth=1,
                linestyle=ls,
            )
            ax[0].plot(
    ...
    df_best10 = df_res_sorted.iloc[:10].sort_values(
        by="total_lap_time", ascending=False
    )
    df_best10["diff"].plot.barh(ax=ax[2], color="b")
    original_xticks = ax[2].get_xticks()
    added_xticklabels = [
        str(datetime.timedelta(seconds=int(x)))
        for x in original_xticks + df_best10["total_lap_time"].min()
    ]
...
def strategy():
    ...
    if button_:
        ...
        _show_pace_graph(df_res=df_res)
