...
def _cvt2seconds(t: str) -> float:
    """Convert lap time written by string to seconds."""
    list_tmp = [
        float(x) if i == 1 else float(x) * 60 for i, x in enumerate(t.split(":"))
    ]
...
def _padding(
    driver: str, total_lap: int, df_season_round_grpd: pd.core.groupby.DataFrameGroupBy
) -> list[float]:
    """Padding nan to the end of the list."""
...
def _read_data() -> pd.DataFrame:
    ...
    df_drivers = pd.read_csv(
        os.path.join(current_dir, "../data/drivers.csv"),
        usecols=["driverId", "driverRef"],
    )
    df_races = pd.read_csv(
        os.path.join(current_dir, "../data/races.csv"),
        usecols=["raceId", "name", "round", "year"],
    )
    df_results = pd.read_csv(
        os.path.join(current_dir, "../data/results.csv"),
...
def race_lap_result():
    ...
    selected_season = st.selectbox(
        "Select season", list_season, key="selected_season_race_lap_result"
    )
    ...
    selected_round = st.selectbox(
        "Select round", list_round, key="selected_round_race_lap_result"
    )
    df_season_round = df_season[df_season["round(gp)"] == selected_round]

    # select drivers to visualize
    list_drivers = (
        df_season_round.groupby("driverRef")["positionOrder"]
        .unique()
    ...
    selected_items = st.multiselect(
        "Select drivers (default = top 5 drivers)",
    ...
    df_season_round_grpd = df_season_round.sort_values(by="lap").groupby("driverRef")[
        "time_sec"
    ]

    # total lap time of eventual winner at each lap
    array_winner_cumsum_lap = np.cumsum(
        _padding(winner, total_lap, df_season_round_grpd)
    )
    list_laptime = list()
    list_gap = list()
    list_driver = list()
    ...
    for driver in df_season_round["driverRef"].unique():
        list_tmp = _padding(driver, total_lap, df_season_round_grpd)
    ...
    df_lap = pd.merge(
        df_lap,
        df_season_round.loc[:, ["driverRef", "positionOrder"]].drop_duplicates(),
        left_on=["driver"],
        right_on=["driverRef"],
        how="left",
    )
    df_lap = df_lap.sort_values(by=["lap", "positionOrder"]).reset_index(drop=True)
    ...
    with col1:
        ...
        fig.update_layout(
            xaxis=dict(tickmode="linear", tick0=1.0, dtick=5.0, tickfont=dict(size=10))
        )
